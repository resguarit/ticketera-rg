...
export default function AppLogo({ title = 'Laravel Starter Kit', logoUrl }: AppLogoProps) {
    ...
    return (
        <>
            <div className="flex aspect-square size-10 items-center justify-center  text-sidebar-primary-foreground overflow-hidden">
                {showFallbackIcon ? (
            ...
            {!isCollapsed && (
                <div className="ml-1 grid flex-1 text-left text-sm">
                    <span className="mb-0.5 truncate leading-tight font-semibold text-lg">{title}</span>
                </div>
            )}
        </>
    );
